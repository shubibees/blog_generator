...
const BlogGeneratorPage: React.FC = () => {
  ...
  const handleGenerateBlog = async (topic: string) => {
    ...
    try {
      ...
      let result: ProcessingResult = { imgURL: null };
      ...
      if (designTask) {
        try {
                ...
                try {
                  ...
                  if (responseData.success) {
                    ...
                    if (!blogSaveResponse.ok) {
                      console.warn('Failed to save blog to database, but continuing...');
                    }
          ...
          // Don't throw here, allow the blog to be displayed without image
          console.warn('Continuing without image due to processing error');
        }
      }
      const newImageUrl = `${result.imgURL}`;
